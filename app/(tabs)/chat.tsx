...
export default function ChatScreen() {
	...
	const MessageBubble = ({ item }: { item: Message }) => {
		...
		return (
			<View style={{ flexDirection: isMe ? 'row-reverse' : 'row', alignItems: 'flex-end', marginVertical: 6 }}>
				...
				>
					<Text style={{ color: isMe ? '#fff' : (isDark ? '#fff' : '#111827') }}>{item.text}</Text>
				</View>
			</View>
		);
	};
	...
	return (
		<View style={{ flex: 1, backgroundColor: isDark ? '#111827' : '#fff' }}>
			...
			<View style={{ paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: isDark ? '#374151' : '#f3f4f6', backgroundColor: isDark ? '#1f2937' : '#fff' }}>
				<View style={{ flexDirection: 'row', alignItems: 'center' }}>
					...
					<View style={{ marginLeft: 12, flex: 1 }}>
						<Text style={{ fontSize: 16, fontWeight: '600', color: isDark ? '#fff' : '#111827' }}>Healify Assistant</Text>
						<Text style={{ fontSize: 12, color: isDark ? '#9ca3af' : '#6b7280' }}>AI health bot</Text>
					</View>
				</View>
				<View style={{ flexDirection: 'row', marginTop: 12, backgroundColor: isDark ? '#374151' : '#F3F4F6', borderRadius: 12, padding: 4 }}>
					<TouchableOpacity
					...
					>
						<Text style={{ textAlign: 'center', color: mode === 'doctor-chat' ? '#fff' : (isDark ? '#fff' : '#111827'), fontWeight: '600' }}>
							Chat with a Doctor
						</Text>
					</TouchableOpacity>
					<TouchableOpacity
					...
					>
						<Text style={{ textAlign: 'center', color: mode === 'certified-response' ? '#fff' : (isDark ? '#fff' : '#111827'), fontWeight: '600' }}>
			...
			>
				<TouchableOpacity style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: isDark ? '#1e3a8a' : '#E0F2FE', alignItems: 'center', justifyContent: 'center' }}>
					<Text style={{ fontSize: 18, color: '#0284C7' }}>📷</Text>
				</TouchableOpacity>
				<View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: isDark ? '#374151' : '#F3F4F6', borderRadius: 20, paddingHorizontal: 12 }}>
					<TextInput
						value={input}
						onChangeText={setInput}
						placeholder="Message"
						placeholderTextColor={isDark ? '#9ca3af' : '#6b7280'}
						style={{ flex: 1, paddingVertical: 10, color: isDark ? '#fff' : '#111827' }}
					/>
